...
def det(tensor):
    ...
    # acq_function  = np.trace(cov_matrix)
def main():
    ...
    y_idx_guess = torch.argmax(training_y)
    theta_t = training_x[y_idx_guess:y_idx_guess+1].clone()

    while (samples_fwd < 40):
        ...
        m52_mll = gp.mlls.ExactMarginalLogLikelihood(m52_gauss_likelihood,m52_model) # finds probability of the function found by GP by comparing to sampled data.
        n_samples = 200
        ...
        with torch.enable_grad(), gp.settings.fast_pred_var(): 
            # Adjust test_x throughout the range and create a prediction Y_pred_m52
            test_x = torch.linspace(start_x, end_x, n_samples, dtype=training_x.dtype)        
        ...
        for i, query_point in enumerate(test_x): # using the test data as the points to sample.   
            ...
            K_xx_new = m52_model.covar_module(training_x_new, training_x_new).evaluate().detach()
